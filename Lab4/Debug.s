...
Debug_Init 
	...
	LDR R1, [R0];
	ORR R1, #0x20; 
	STRB R1, [R0]; Turns on the clock for port F
	...
	LDR R1, [R0];
	ORR R1, #0x02 ; sets PF1 as LED output on tm4c
	STRB R1, [R0] ; stores back to data register

	LDR R0, =GPIO_PORTF_DEN_R;
	LDR R1, [R0];
	ORR R1, #0x2 ; digital enable for port F pins 1,0 and but 6;
	STRB R1, [R0];
	
	
	

	;initlize max time and min time
	
	LDR R0, =MaximumTime;
	MOV R1, #0x00000000; largest possible 32 bit unsigned
	STR R1, [R0];
	
	LDR R0, =MinimumTime;
	MOV R1, #0xFFFFFFFF;
	STR R1, [R0]; ;smallest possible 32 bit unsigned.
	


; sets min time to lowest possible 32 bit unsigned.
...
;initilize count TO 0
	...
	LDR R0, =Loop;
	MOV R2, #0;
	STRB R2, [R0]; initilize countdown for beat
	
	;initlilize N
	
	LDR R0, =Elements;
	MOV R2, #168;
	STR R2, [R0]; timeslot now contains 168
	
	
	;initilize dump
		;LDR
	LDR R0, =DumpBuf ;
	MOV R2, #0xFF ; move into each element in dumpbuf
	STRB R2, [R0];
	MOV R1, #0;
LOOP3	CMP R1, #200;
	BEQ DONE ;
	...
	;STR R0, [R1];
	  	...
	  	;;; Lab 4, N=168
;EID1=ZMH345, EID2=AR68433
;Option 2: Dump if R0 bit 6 low and bit 0 high
;Implement HeartBeat on PF1

DONE    POP {R0,R1,R2,PC};
...
;Calculate intrusiveness is T/2.5ms = ???
	...
	LDR R1, =Count;
	LDRB R2, [R1]; contains contents of count (should be 0) used as index for array
	CMP R2, #168; array element numbers, see if full
	BEQ done ; if equal array is now full
	
	
	;R0 bit 0 is high

	;data needed
	AND R4, R0,#0x41; isolates bit 6 and bit 0;
	CMP R4, #0x01 ; bit 6 is low and bit 0 is high. if true add to array.
	BNE done;
	
	LDR R5, =DumpBuf;
	
	STRB R0, [R5, R2]; stores value at appropriate location in array
	
	ADD R2, R2, #1; Increments to the next array element
	STRB R2, [R1]; place next count value at count address (array index)
...
;you write this
	 ;skip first call
	LDR R0, =Elements;
	LDR R0, [R0]; now has N in it;
	CMP R0, #168; if r0=168 then we skip
	BEQ first
	...
	;make comparisons and find new min/max
	;elapsed time = last time - now time      R2 has address of last time, r1, has current time, r4 will contain elapsed time
	LDR R1, =TIMER2_TAR_R;
	LDR R1, [R1] ; contains current time (now time)
	LDR R2, =LastTime;
	LDR R3, [R2];  contains lastTime value
	
	;elapsed time
	STR R1, [R2]; current time becomes last time
	SUB R4, R3, R1; elapsed time
	...
	LDR R5, =MinimumTime;
	LDR R6, [R5] ;contains the min time value (ffffffffff)
	CMP R4, R6;
	BLO updateminimum

check
	LDR R5, =MaximumTime;
	LDR R6, [R5]; contains max time value 0
	CMP R4, R6;
	BHI updatemaximum
	B increment
	;B updatetime;

updateminimum STR R4, [R5];
				;B updatetime
	;LDR R5, =MaximumTime;
	;LDR R6, [R5]; contains max time value 0
	;CMP R4, R6;
...
;you write this    
	...
	LDR R2, =Loop;
	LDRB R4, [R2]; ;contains the counter
	CMP R4, #200;
	BNE turnoff


;will only be called after beat is called 200 times.
	LDR R0, =GPIO_PORTF_DATA_R ;
	LDRB R1, [R0];
; only toggle when counter reaches 0.
	EOR R1, #0x02; toggle for heartbeat
	STRB R1, [R0] ; Stores toggle back to data register
;B ending
